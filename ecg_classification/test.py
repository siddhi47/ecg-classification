...
logger.add("test.log", rotation="1 MB", compression="zip")

def parse_args():
    args = ArgumentParser(description="Test the model for ECG classification.")
    args.add_argument(
        "--data", type=str, default="dataset", help="Path to the data folder."
    )
...
def main(args):
    ...
    if len(dataloader) == 0:
        logger.info(f"Could not find any data in {args.data}")
        return
    ...
    if any([args.model in file for file in os.listdir(args.save)]):
        ...
        logger.info(f"No saved model found for {args.model}.")
        return
    ...
    logger.info(f"Precision: {true_positive / (true_positive + false_positive)}")
    logger.info(f"Recall: {true_positive / (true_positive + false_negative)}")
    logger.info(
        f"F1 score: {2 * true_positive / (2 * true_positive + false_positive + false_negative)}"
    )
    logger.info(f"Specificity: {true_negative / (true_negative + false_positive)}")
    ...
    for idx, (i, j, k) in enumerate(zip(cm.sum(axis=1), cm.sum(axis=0), cm.diagonal())):
        f1 = 2 * k / (i + j)
        logger.info(f"F1 score for class {idx}: {f1}")
        f1_list.append(f1)
    logger.info(f"Mean F1 score: {np.mean(f1_list)}")
